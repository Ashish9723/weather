...
const API_KEY = "de3ffc19530024cb3027e2fbddaf281f";  //API key from OpenweatherMap API
    ...
    if(index === 0) { //html for main weather card
        ...
        <div class="current-weather">
            <div class ="details">
                    ...
                    <h4>Tempature: ${(weatherItem.main.temp - 273.15).toFixed(2)} °C </h4>
...
const getWeatherDetails = (cityName, lat, lon)  => {
    ...
    fetch(WEATHER_API_URL).then(res => res.json()).then(data => {
        ...
        //filter  the forcast to get only one forecast per day
...
const getCityCoordinates = () => {
    ...
    fetch(GEOCODING_API_URL).then(res  => res.json()).then(data => {
        if(!data.length) return  alert(`no coordinates found ${cityName}`);
